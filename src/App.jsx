...
export default function App() {
  ...
  useEffect(() => { 
    if (typeof window !== 'undefined') window.localStorage.setItem('tmr_v19_signatures', JSON.stringify(signatures)); 
  }, [signatures]);
  
  useEffect(() => { 
    if (typeof window !== 'undefined') window.localStorage.setItem('tmr_v19_categories', JSON.stringify(categories)); 
  }, [categories]);
  
  useEffect(() => { 
    if (typeof window !== 'undefined') window.localStorage.setItem('tmr_v19_allReports', JSON.stringify(allReports)); 
  }, [allReports]);
  ...
  const handleAddActiveItem = () => {
    ...
    if (activeType === 'utama') {
      if (isAddingSusulan) {
        if (!susulanValidDate) {
          showConfirm("Mohon pilih Tanggal Validasi untuk pendapatan susulan!", null);
          return;
        }
  ...
  const getDaysArray = () => {
    ...
    const days = Array.from({length: daysInMonth}, (_, i) => {
      ...
      const utamaTotal = hasUtama && dayData.utama?.formData
      ...
      Object.keys(dayData).forEach(k => {
        if ((k === 'lain' || k.startsWith('lain_')) && Array.isArray(dayData[k].activeItems) && dayData[k].activeItems.length > 0) {
          const docTotal = dayData[k].formData
                        ...
                        {d.hasUtama && (
                          <div className="w-full bg-green-50 border border-green-200 rounded shadow-sm flex flex-col overflow-hidden shrink-0">
                            <div className="bg-green-500 text-white flex justify-between items-center px-1.5 py-0.5">
                              ...
                              {d.utamaSequence && d.utamaSequence !== '...' && <span className="text-[9px] font-bold">{d.utamaSequence}</span>}
                            </div>
                            <div className="text-[9px] sm:text-[10px] font-black text-green-800 text-right px-1.5 py-0.5 truncate" title={`Rp ${formatRp(d.utamaTotal)}`}>
                        ...
                        {d.lainDocs.map((lainDoc, index) => (
                          <div key={index} className="w-full bg-purple-50 border border-purple-200 rounded shadow-sm flex flex-col overflow-hidden shrink-0">
                            <div className="bg-purple-500 text-white flex justify-between items-center px-1.5 py-0.5">
                              <span className="text-[9px] font-bold">SU/L</span>
                              {lainDoc.sequence && lainDoc.sequence !== '...' && <span className="text-[9px] font-bold">{lainDoc.sequence}</span>}
                            </div>
                            <div className="text-[9px] sm:text-[10px] font-black text-purple-800 text-right px-1.5 py-0.5 truncate" title={`Rp ${formatRp(lainDoc.total)}`}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mt-3">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex-1 border border-dashed border-gray-300 rounded-lg bg-gray-50 p-2 flex items-center overflow-x-auto min-w-0">
                    <span className="font-mono font-bold text-gray-600 text-xs sm:text-sm whitespace-nowrap truncate">{computedStsuNo || 'Preview...'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className={`p-3 rounded-xl border flex flex-col justify-center shadow-inner items-end ${activeType === 'utama' ? 'bg-green-50 border-green-200' : 'bg-purple-50 border-purple-200'}`}>
                ...
                <span className={`text-lg sm:text-xl font-black truncate max-w-full ${activeType === 'utama' ? 'text-green-800' : 'text-purple-800'}`} title={`Rp ${formatRp(grandTotal)}`}>
