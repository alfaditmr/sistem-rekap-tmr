...
export default function App() {
  ...
  const defaultReportData = {
    "utama": {
      ...
      activeItems: [
        {catId: "cat_1", itemId: "item_1a"},
        {catId: "cat_2", itemId: "item_2a"},
        {catId: "cat_3", itemId: "direct"},
        {catId: "cat_5", itemId: "item_1"}, {catId: "cat_5", itemId: "item_2"}, {catId: "cat_5", itemId: "item_8"}
      ],
      formData: {}
    }
  };
  ...
  // --- HELPER UNTUK KODE INPUT (BAIK FIREBASE MAUPUN LOKAL) ---
  ...
  const handleTarikData3A = async () => {
    if (!ipRobot) {
      alert("IP PC Robot belum diatur! Silakan atur IP terlebih dahulu di menu Master (Pengaturan Sistem Lokal).");
      return;
    }
    
    setIsFetching3A(true);
    try {
      const url = `http://${ipRobot}:5000/api/tarik_rekon_3a?tanggal=${reportDate}`;
      const response = await fetch(url);
      
      if (!response.ok) {
        throw new Error(`Gagal menghubungi server lokal (Status: ${response.status})`);
      }
      
      const data = await response.json();
      const rekon = data.rekon_data || {};

      const gateData = rekon["GATE"] || {};
      const tvmData = rekon["TVM"] || {};
      const onlineData = rekon["MERCHANT_PAGE"] || {};
      ...
      const ticketDictionary = {
        ...
        "8": "Taman Satwa Anak", "9": "Pusat Primata Schmutzer", "10": "Sepeda Tunggal",
        "12": "Sepeda Ganda", "14": "Sepeda Air", "20": "Motor",
      ...
      const ensureCategory = (catId, catName) => {
        ...
        if (!cat) {
          cat = { id: catId, name: catName, type: 'utama', items: [] };
      ...
      const catGate = ensureCategory('cat_5', 'E-ticketing New Gate');
      const catOnline = ensureCategory('cat_6', 'Ticket online');
      const catTvm = ensureCategory('cat_7', 'TVM');
      ...
      updateCurrentReport(prev => {
        ...
        return { ...prev, activeItems: newActiveItems, formData: newFormData };
      });
      
      alert("✅ Seluruh data 3A (Gate, TVM, Tiket Online) berhasil ditarik dan di-mapping otomatis ke dalam STSU Utama!");

    } catch (error) {
      console.error("Fetch Error:", error);
      alert(`❌ Gagal menarik data 3A:\n${error.message}\n\nPastikan:\n1. IP Address PC benar (${ipRobot})\n2. PC Robot dan Tablet berada di jaringan WiFi yang sama.\n3. Program server API Python sedang berjalan.`);
    } finally {
      ...
      {activeTab === 'settings' && (
        <div className="max-w-4xl mx-auto px-4 py-6 no-print space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <h2 className="text-lg font-bold mb-4 text-gray-800 flex items-center gap-2"><Settings size={20} className="text-blue-500"/> Pengaturan Sistem Lokal</h2>
            <div className="space-y-3 bg-gray-50 p-4 rounded-lg border border-gray-100">
              <h3 className="font-semibold text-gray-700 text-sm border-b pb-2 flex items-center gap-2">
                <Cloud size={16} className="text-blue-500" /> Koneksi API Robot 3A
              </h3>
              <div>
                <label className="text-xs text-gray-500 uppercase block mb-1 font-bold">IP Address PC Loket (Contoh: 192.168.1.15)</label>
                <div className="flex gap-2">
                  <input 
                    type="text" 
                    value={ipRobot} 
                    onChange={(e) => setIpRobot(e.target.value)} 
                    placeholder="192.168.1.15" 
                    className="w-full max-w-sm border border-gray-300 rounded p-2.5 text-sm outline-none focus:border-blue-500 font-mono font-bold bg-white focus:ring-2 focus:ring-blue-500/50" 
                  />
                </div>
                <p className="text-[11px] text-gray-500 mt-1.5 font-medium leading-relaxed">
                  IP ini digunakan untuk menarik rekon data (Gate, TVM, Ticket Online) secara otomatis dari PC lokal loket via WiFi.
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {activeType === 'utama' && (
                  <>
                    ...
                    <button
                      onClick={handleTarikData3A}
                      disabled={isFetching3A}
                      className="flex items-center gap-2 text-sm font-bold bg-blue-100/80 text-blue-700 px-3 py-1.5 rounded-lg border border-blue-300 hover:bg-blue-200 transition-colors shadow-sm disabled:opacity-50"
                      title="Tarik Seluruh Rekon 3A Otomatis (Gate, TVM, Tiket Online)"
                    >
                      {isFetching3A ? <RefreshCw size={16} className="animate-spin" /> : <span>🤖</span>}
                      {isFetching3A ? 'Menarik Data...' : 'Tarik Data 3A'}
                    </button>
